...
public final class PlanarYUVLuminanceSource extends LuminanceSource {
	...
	public Bitmap renderCroppedGreyscaleBitmap() {
		/*
		 * int width = getWidth(); int height = getHeight(); int[] pixels = new
		 * int[width * height]; byte[] yuv = yuvData; int inputOffset = top *
		 * dataHeight + left; for (int y = 0; y < height; y++) { int
		 * outputOffset = y * width; for (int x = 0; x < width; x++) { int grey
		 * = yuv[inputOffset + x] & 0xff; pixels[outputOffset + x] = 0xFF000000
		 * | (grey * 0x00010101); } inputOffset += dataHeight; }
		 * 
		 * Bitmap bitmap = Bitmap.createBitmap(width, height,
		 * Bitmap.Config.ARGB_8888); bitmap.setPixels(pixels, 0, width, 0, 0,
